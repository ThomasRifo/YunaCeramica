import { Head, Link, router } from "@inertiajs/react";
import { useState, useEffect, useRef } from "react";

export default function MayoristaIndex({ productos, subcategorias, filtros }) {
  const [busqueda, setBusqueda] = useState(filtros?.busqueda || '');
  const timeoutRef = useRef(null);
  const inputRef = useRef(null);
  const wasFocusedRef = useRef(false);

  // Búsqueda dinámica con debounce
  useEffect(() => {
    // Solo ejecutar si la búsqueda realmente cambió (no en el mount inicial si ya hay filtros)
    const busquedaActual = busqueda.trim();
    const busquedaAnterior = filtros?.busqueda || '';
    
    // Si la búsqueda no cambió, no hacer nada
    if (busquedaActual === busquedaAnterior) {
      return;
    }

    // Limpiar timeout anterior si existe
    if (timeoutRef.current) {
      clearTimeout(timeoutRef.current);
    }

    // Crear nuevo timeout para ejecutar la búsqueda después de 500ms sin escribir
    timeoutRef.current = setTimeout(() => {
      const params = {};
      
      // Preservar filtro de subcategoría si existe
      if (filtros?.subcategoria) {
        params.subcategoria = filtros.subcategoria;
      }
      
      // Agregar búsqueda si hay texto
      if (busquedaActual) {
        params.busqueda = busquedaActual;
      }
      
      // Guardar si el input tiene foco antes de navegar
      wasFocusedRef.current = document.activeElement === inputRef.current;
      
      router.get('/mayorista', params, {
        preserveState: true,
        preserveScroll: false,
        replace: true,
      });
    }, 500); // Esperar 500ms después de que el usuario deje de escribir

    // Cleanup function
    return () => {
      if (timeoutRef.current) {
        clearTimeout(timeoutRef.current);
      }
    };
  }, [busqueda]);

  const handleBusquedaChange = (e) => {
    setBusqueda(e.target.value);
    if (document.activeElement === e.target) {
      wasFocusedRef.current = true;
    }
  };

  // Efecto para restaurar el foco después de que el componente se monte/actualice
  useEffect(() => {
    if (wasFocusedRef.current && inputRef.current) {
      const timer = setTimeout(() => {
        inputRef.current?.focus();
        const length = inputRef.current.value.length;
        inputRef.current.setSelectionRange(length, length);
        wasFocusedRef.current = false;
      }, 100);
      return () => clearTimeout(timer);
    }
  }, [productos, filtros]);

  const handleFiltro = (tipo, valor) => {
    const params = { ...filtros };
    if (valor) {
      params[tipo] = valor;
    } else {
      delete params[tipo];
    }
    router.get('/mayorista', params, {
      preserveState: true,
      preserveScroll: true,
    });
  };

  const schemaData = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    "name": "Catálogo Mayorista | Yuna Cerámica",
    "description": "Explorá nuestro catálogo de productos disponibles para venta mayorista. Piezas artesanales y bizcochados con descuentos por cantidad.",
    "url": "https://yunaceramica.com/mayorista",
    "publisher": {
      "@type": "Organization",
      "name": "Yuna Cerámica",
      "logo": "https://yunaceramica.com/storage/uploads/yunalogowhite.webp"
    }
  };

  return (
    <>
      <Head>
        <title>Venta Mayorista</title>
        
        {/* Metas Básicas de SEO */}
        <meta 
          name="description" 
          content="Venta mayorista de piezas de cerámica artesanal y bizcochados en Yuna Cerámica. Precios especiales y descuentos por cantidad." 
        />
        <meta 
          name="keywords" 
          content="cerámica mayorista, venta mayorista cerámica, bizcochados mayorista, piezas para pintar mayorista, cerámica Cipolletti, cerámica Neuquén" 
        />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href="https://yunaceramica.com/mayorista" />

        {/* Open Graph */}
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://yunaceramica.com/mayorista" />
        <meta property="og:title" content="Venta Mayorista - Cerámica Artesanal y Bizcochados | Yuna Cerámica" />
        <meta property="og:description" content="Catálogo exclusivo de piezas y bizcochados con precios mayoristas por cantidad." />
        <meta property="og:image" content="https://yunaceramica.com/storage/uploads/productos.webp" />
        <meta property="og:locale" content="es_AR" />

        {/* Twitter Card */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="Venta Mayorista | Yuna Cerámica" />
        <meta name="twitter:description" content="Catálogo de productos con precios mayoristas por cantidad." />
        <meta name="twitter:image" content="https://yunaceramica.com/storage/uploads/productos.webp" />

        {/* Schema.org */}
        <script type="application/ld+json">
          {JSON.stringify(schemaData)}
        </script>
      </Head>

      <div className="min-h-screen bg-gray-50">
        {/* Hero Section */}
        <div className="relative h-[50vh] w-full">
          <img 
            src="/storage/uploads/productos.webp" 
            alt="Portada productos mayoristas" 
            className="object-cover w-full h-full object-[center_50%]" 
          />
          <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center text-white text-center px-4">
            <h1 className="mt-16 text-6xl md:text-7xl font-bold">Venta Mayorista</h1>
            <p className="mt-2 text-lg md:text-3xl">Piezas y bizcochados con precios especiales por cantidad para revendedores y talleres</p>
          </div>
        </div>

        {/* Filtros y Búsqueda */}
        <div className="max-w-7xl mx-auto px-4 py-16">
          <div className="mb-8 space-y-4">
            {/* Búsqueda */}
            <div className="flex gap-2">
              <input
                ref={inputRef}
                type="text"
                value={busqueda}
                onChange={handleBusquedaChange}
                placeholder="Buscar productos mayoristas..."
                className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>

            {/* Filtros por Subcategoría */}
            {subcategorias && subcategorias.length > 0 && (
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => handleFiltro('subcategoria', null)}
                  className={`px-4 py-2 rounded-lg ${
                    !filtros?.subcategoria 
                      ? 'bg-blue-600 text-white' 
                      : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                  }`}
                >
                  Todas
                </button>
                {subcategorias.map((subcategoria) => (
                  <button
                    key={subcategoria.id}
                    onClick={() => handleFiltro('subcategoria', subcategoria.id)}
                    className={`px-4 py-2 rounded-lg ${
                      filtros?.subcategoria == subcategoria.id
                        ? 'bg-blue-600 text-white'
                        : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                    }`}
                  >
                    {subcategoria.nombre}
                  </button>
                ))}
              </div>
            )}
          </div>

          {/* Grid de Productos */}
          {productos && productos.data && productos.data.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {productos.data.map((producto) => {
                const imagenPrincipal = producto.imagenes && producto.imagenes.length > 0 
                  ? `/storage/productos/${producto.imagenes[0].urlImagen}`
                  : '/storage/uploads/placeholder.jpg';
                
                const precioMinorista = producto.descuento 
                  ? producto.precio * (1 - producto.descuento / 100)
                  : producto.precio;

                const precioMayorista = producto.descuento_mayorista
                  ? producto.precio * (1 - producto.descuento_mayorista / 100)
                  : producto.precio;

                return (
                  <div
                    key={producto.id}
                    onClick={() => router.visit(`/productos/${producto.slug}`, { preserveState: false, preserveScroll: false })}
                    className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow cursor-pointer flex flex-col"
                  >
                    <div className="relative aspect-square">
                      <img
                        src={imagenPrincipal}
                        alt={producto.nombre}
                        className="w-full h-full object-cover"
                      />
                      {producto.descuento_mayorista && (
                        <div className="absolute top-2 right-2 bg-black text-white px-2 py-1 rounded text-xs font-semibold shadow-sm">
                          -{producto.descuento_mayorista}% Mayorista
                        </div>
                      )}
                      {producto.stock === 0 && (
                        <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                          <span className="text-white text-xl font-bold">SIN STOCK</span>
                        </div>
                      )}
                    </div>
                    <div className="p-4 flex-1 flex flex-col justify-between">
                      <div>
                        <h3 className="font-semibold text-2xl mb-1 line-clamp-2">{producto.nombre}</h3>
                        {producto.cant_minima_mayorista && (
                          <p className="text-xs font-medium text-gray-500 mb-2">
                            Mínimo mayorista: +{producto.cant_minima_mayorista} unidades
                          </p>
                        )}
                      </div>
                      <div className="mt-2 pt-2 border-t border-gray-100 flex items-baseline justify-between">
                        <div>
                          <span className="text-xs text-gray-500 block">Mayorista:</span>
                          <span className="text-xl font-bold text-gray-900">
                            ${precioMayorista.toLocaleString('es-AR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                          </span>
                        </div>
                        <div className="text-right">
                          <span className="text-xs text-gray-400 block">Minorista:</span>
                          <span className="text-sm text-gray-400 font-medium line-through">
                            ${precioMinorista.toLocaleString('es-AR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
                );
              })}
            </div>
          ) : (
            <div className="text-center py-12">
              <p className="text-gray-500 text-lg">No se encontraron productos disponibles para venta mayorista.</p>
            </div>
          )}

          {/* Paginación */}
          {productos && productos.links && (
            <div className="mt-8 flex justify-center">
              <div className="flex gap-2 items-center">
                {productos.links.map((link, index) => {
                  let label = link.label;
                  if (label === 'pagination.previous' || label === '&laquo; Previous') {
                    label = '← Anterior';
                  } else if (label === 'pagination.next' || label === 'Next &raquo;') {
                    label = 'Siguiente →';
                  } else if (label.includes('&laquo;')) {
                    label = label.replace('&laquo;', '←');
                  } else if (label.includes('&raquo;')) {
                    label = label.replace('&raquo;', '→');
                  }
                  
                  return (
                    <button
                      key={index}
                      onClick={() => link.url && router.get(link.url)}
                      className={`px-4 py-2 rounded border ${
                        link.active
                          ? 'bg-blue-600 text-white border-blue-600'
                          : 'bg-white text-gray-700 hover:bg-gray-100 border-gray-300'
                      } ${!link.url ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                    >
                      {label}
                    </button>
                  );
                })}
              </div>
            </div>
          )}
        </div>
      </div>
    </>
  );
}
